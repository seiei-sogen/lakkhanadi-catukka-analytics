...
- 補助 TypeScript スクリプト（Pali 表記正規化）は Skill ホストの前処理段で実行可能
...
- **パターン**: **Prompt-Driven Skill Architecture（プロンプト駆動 Skill アーキテクチャ）**
  ...
  - 補助: TypeScript スクリプト（前処理のみ、Skill 本体は依存しない）
  - 単一 SKILL.md で Claude Code skills と Codex CLI skills の両方をカバー
- **選択理由**:
  - REQ-403: Skill の主体はプロンプト・markdown で構成、TypeScript は補助スクリプトに留める
...
- **ロジック層**:
  ...
  - `normalizer`: Pali 表記の標準化指示テーブル（TS スクリプト不在環境のフォールバック）

### 補助構成: TypeScript 前処理スクリプト 🟡

**信頼性**: 🟡 *設計ヒアリングQ3 で「TS 補助スクリプトで事前正規化」を選択。ただし REQ-408 の `--tools ""` 制約との両立は実装フェーズで確認*

- **位置**: `skills/lakkhanadi-catukka/scripts/normalize.ts`
- **責務**: ユーザー入力に含まれる Pali 用語の異形・IAST 揺れを標準表記へ正規化（REQ-107, 108）
- **呼び出し方式**: Skill ホスト（Claude Code / Codex CLI）の前処理フックがあればそこで実行。無い環境では `prompt` 内の Pali 正規化テーブルがフォールバックとして同等の効果を出す（二段冗長）
- **dependencies**: なし（Node.js 標準ライブラリのみ）
- **備考**: REQ-408 で AI コマンドが `--tools ""` で起動するため、TS スクリプトを LLM が自発的に呼ぶことはない。前処理フックを持たないホストでは Markdown 内テーブルが効く
...
graph TB
    ...
    PreProc[TS 補助<br/>scripts/normalize.ts]
...
- Skill ホストが前処理フックを持つ場合、TS スクリプトで Pali 正規化を実行してから入力を SKILL.md に渡す
...
- 補助スクリプト `scripts/normalize.ts` を含めない最小構成（プロンプト内テーブルのみ）でも動作可能（REQ-408 への安全側の保証）
...
- **REQ-403**: 主体はプロンプト・markdown であり、TypeScript は補助
- **REQ-404**: 出力言語は日本語固定
...
- 🟡 はスケーラビリティ・可用性の一般推測（Skill 特性上、独自の制約は存在しないため）と TS スクリプトの呼び出し方式（実装フェーズで確認すべき項目）の 2 件のみ
