...
> **2026-05-07 注記**: 本記録は当時の議論をそのまま保存している。記録中の「TS / TypeScript / normalize.ts」表現はすべて補助スクリプトの実装言語を Rust に変更する決定（REQ-403 改訂）後、`Rust / normalize.rs` に読み替える。設計判断（軽量変換器・二段冗長・公開関数シグネチャ等）は言語変更後も維持される。最新の実装契約は `architecture.md` / `interfaces.ts` ヘッダ参照。
